...
界面采用左侧四分之一语音工作区与右侧四分之三画布区。工作区依次展示麦克风与字幕、自动居中滚动的任务队列、针对当前任务的系统反问，以及仅由用户明确语音保存的版本库。系统不会自动把操作快照收录进版本库。
...
当前内置基线包含 99 个规范词、49 个别名、18 条高精度错词规则和 71 组常见同音混淆，并使用 `pinyin-pro` 为任意中文业务词生成完整拼音。运行时词典会自动加入当前图标题、所有节点、连线标签和最近命令，因此不受内置词表数量上限约束。
...
| 指令理解准确与容错 | 本地 ASR 校准、当前画布上下文匹配、候选消歧             |
...
项目不提供 Mock AI 或自动回退。Agent 复杂生成与上下文修改必须配置真实 OpenAI-compatible Provider：
