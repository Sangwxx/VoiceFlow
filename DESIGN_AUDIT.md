# VoiceFlow 设计审计

## 已实现能力

- 纯语音创建、编辑、排版、美化、版本管理和导出图表。
- Fast、Simple、Workflow、Agent 四级命令路由。
- Fast Path 常用命令本地直通与实时延迟指标。
- 不调用 AI 的本地 ASR 校准：可扩展领域词典、高精度错词映射、完整汉字拼音、编辑距离、候选竞争和当前画布上下文。
- 多候选目标按本地相关性排序并确定性选择。
- LLM 主导通用结构图规划，以及 Agent Operation 批次的校验与直接应用。
- Diagram 运行时校验、撤销/重做、持久版本、JSON/SVG/PNG 导出。
- 仅支持真实 OpenAI-compatible Provider；未配置和请求失败均明确报错。
- TypeScript、ESLint、Prettier、Vitest、生产构建和 GitHub Actions。

## 题目关键问题解决方案

### 响应延迟

确定性高频命令不调用 AI，由 Fast Path 本地执行。Simple 和 Workflow 规则路径同样优先于 Agent。系统记录真实执行耗时，并在界面展示各路径平均耗时与 Fast Path 命中率。

### 语言歧义

系统使用领域词典与命令别名作为基线，并动态加入当前 Diagram 标题、节点、连线标签和最近命令。完整汉字拼音、编辑距离、候选分数差值、已识别词保护和重叠冲突消解共同控制替换。目标存在多个候选时，系统按本地相关性排序选择最佳匹配并继续执行。

### 复杂指令安全

完整新图由 LLM 根据受约束 Prompt 生成紧凑蓝图；基础图形与确定性编辑保持本地执行，上下文修改只能转换为白名单 Operation。所有模型结果通过运行时校验后直接提交为一条可撤销历史记录；未配置 AI 时才使用本地规划器降级。

## 计划与最终实现对照

| 计划能力             | 最终状态 | 说明                                     |
| -------------------- | -------- | ---------------------------------------- |
| 纯语音绘图           | 已完成   | 画布禁用鼠标、键盘和触摸编辑             |
| 常用命令低延迟直通   | 已完成   | Fast Path 本地执行并展示耗时             |
| 语言纠错与目标匹配   | 已完成   | 本地 ASR 校准与确定性最佳匹配            |
| 系统反问与确认流程   | 已删除   | 避免阻塞连续语音任务，校验通过后直接执行 |
| 新旧版本左右双画布   | 暂缓     | 截止时间内优先保证核心流程稳定           |
| 云端 ASR / Whisper   | 暂缓     | 避免引入额外服务和现场网络风险           |
| 后端同步与协作       | 暂缓     | 超出本次纯语音绘图核心范围               |
| PDF/PPT/draw.io 导出 | 暂缓     | 当前提供 JSON、SVG 和 PNG                |

## 已知边界

- 浏览器首次麦克风授权属于浏览器安全要求，需要用户手动允许。
- Agent 任意主题生成和自由修改必须配置真实 Provider，不提供 Mock 或自动回退。
- 浏览器暴露的 `VITE_` 密钥仅适合本地演示，生产环境需要后端代理。
