...
- 自由画布导出 SVG 时必须直接序列化 Scene 中的标准 SVG 图元，不得使用包含 `foreignObject` 的网页截图式 SVG，避免外部查看器显示空白。
- 专业图表导出 SVG 时必须从 Diagram 数据和本地布局结果直接生成标准矢量节点、连线、箭头与文字，不得依赖 React Flow DOM 截图或 `foreignObject`。
